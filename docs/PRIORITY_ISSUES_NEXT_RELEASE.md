...
This document contains the top 5 priority issues identified for the next release. These issues can be created manually or via the workflow `.github/workflows/create-priority-issues.yml`.

## Summary

| # | Title | Priority | Effort | Impact | Labels |
|---|-------|----------|--------|--------|--------|
| 1 | 🔒 Enhance security headers based on ZAP scan findings | High | Medium (4-6h) | Critical | `type:security`, `priority:high`, `size:medium`, `domain:infrastructure` |
| 2 | 🔷 Fix TypeScript explicit return type warnings (8 locations) | High | Small (1-2h) | High | `type:refactor`, `priority:high`, `size:small`, `domain:frontend` |
| 3 | ⚡ Optimize bundle size with Three.js code-splitting | High | Medium (4-6h) | High | `type:performance`, `priority:high`, `size:medium`, `domain:infrastructure` |
| 4 | 🧪 Increase App.tsx test coverage from 62% to 80%+ | High | Small (2-3h) | High | `type:test`, `priority:high`, `size:small`, `domain:testing` |
| 5 | 📦 Update dependencies and run security audit | Medium | Small (1-2h) | Medium | `type:chore`, `priority:medium`, `size:small`, `domain:infrastructure` |

**Total Estimated Effort:** 13-19 hours across 5 independent, parallelizable tasks

---

## Issue 1: 🔒 Enhance security headers based on ZAP scan findings
...
- **Related Issue:** #320
- **Labels:** `type:security`, `priority:high`, `size:medium`, `domain:infrastructure`

---

## Issue 2: 🔷 Fix TypeScript explicit return type warnings (8 locations)
...
- **Labels:** `type:refactor`, `priority:high`, `size:small`, `domain:frontend`

---

## Issue 3: ⚡ Optimize bundle size with Three.js code-splitting
...
dist/assets/three-rZOyD2Np.js   1,102.04 kB │ gzip: 309.34 kB
```
...
- **Labels:** `type:performance`, `priority:high`, `size:medium`, `domain:infrastructure`

---

## Issue 4: 🧪 Increase App.tsx test coverage from 62% to 80%+
...
- **Labels:** `type:test`, `priority:high`, `size:small`, `domain:testing`

---

## Issue 5: 📦 Update dependencies and run security audit
...
- `@types/react`: 19.2.2 → 19.2.3 (patch update available)
...
- [ ] Verify SBOM quality check still passes (score ≥ 7.0/10)
...
- **Type:** Maintenance
- **Labels:** `type:chore`, `priority:medium`, `size:small`, `domain:infrastructure`
...
3. All 5 issues will be created automatically with proper labels
...
gh issue create \
  ...
  --body-file docs/issues/issue-1-security-headers.md \
  --label "type:security,priority:high,size:medium,domain:infrastructure"

# Issue 2 - TypeScript
gh issue create \
  --title "🔷 Fix TypeScript explicit return type warnings (8 locations)" \
  --body-file docs/issues/issue-2-typescript.md \
  --label "type:refactor,priority:high,size:small,domain:frontend"

# Issue 3 - Bundle Size
gh issue create \
  --title "⚡ Optimize bundle size with Three.js code-splitting" \
  --body-file docs/issues/issue-3-bundle-size.md \
  --label "type:performance,priority:high,size:medium,domain:infrastructure"

# Issue 4 - Test Coverage
gh issue create \
  --title "🧪 Increase App.tsx test coverage from 62% to 80%+" \
  --body-file docs/issues/issue-4-test-coverage.md \
  --label "type:test,priority:high,size:small,domain:testing"

# Issue 5 - Dependencies
gh issue create \
  --title "📦 Update dependencies and run security audit" \
  --body-file docs/issues/issue-5-dependencies.md \
  --label "type:chore,priority:medium,size:small,domain:infrastructure"
```

### Option 3: Manually via GitHub UI
Copy the content from each issue section above and create issues manually through the GitHub web interface.

---

## Priority Scoring

Each issue was scored using the formula:
```
Priority Score = (Impact × 2) + Urgency + Effort Bonus
Where: S=+3, M=+2, L=+1, XL=0
```

| Issue | Impact | Urgency | Effort | Score | Rank |
|-------|--------|---------|--------|-------|------|
| Security Headers | 5 | 5 | M (+2) | 17 | 1 |
| TypeScript Typing | 4 | 4 | S (+3) | 15 | 2 |
| Bundle Size | 4 | 4 | M (+2) | 14 | 3 |
| Test Coverage | 4 | 3 | S (+3) | 14 | 4 |
| Dependency Updates | 3 | 3 | S (+3) | 12 | 5 |

All issues are designed to be:
- ✅ Small or medium effort (completable in 1-6 hours)
- ✅ Independent (can be worked on in parallel)
- ✅ Well-defined with clear acceptance criteria
- ✅ High impact on code quality, security, or performance
- ✅ Aligned with ISMS policies
