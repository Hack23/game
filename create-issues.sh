...
# Script to create 5 priority GitHub issues for the game repository
# This script uses the GitHub CLI (gh) to create issues from the specifications

set -e

REPO="Hack23/game"

echo "Creating 5 priority issues for $REPO..."
echo ""

# Issue 1: Security Findings
echo "Creating Issue 1: Fix 12 High-Severity Security Findings from ZAP Scan..."
gh issue create \
  --repo "$REPO" \
...
echo "Creating Issue 2: Optimize Three.js Bundle Size..."
gh issue create \
  --repo "$REPO" \
  --title "⚡ Optimize Three.js Bundle Size from 1.1MB to <500KB" \
  --label "performance,priority:high,size:medium,type:performance" \
  --body "## 🎯 Objective
...
echo "Creating Issue 3: Update 3 Outdated Dependencies..."
gh issue create \
  --repo "$REPO" \
  --title "📦 Update 3 Outdated Dependencies to Latest Versions" \
  --label "dependencies,priority:high,size:small,type:chore" \
  --body "## 🎯 Objective
...
echo "Creating Issue 4: Add Comprehensive API Documentation..."
gh issue create \
  --repo "$REPO" \
  --title "📝 Add Comprehensive API Documentation with Architecture Diagrams" \
  --label "documentation,priority:medium,size:medium,type:docs" \
  --body "## 🎯 Objective
...
echo "Creating Issue 5: Increase App.tsx Test Coverage..."
gh issue create \
  --repo "$REPO" \
  --title "✅ Increase App.tsx Test Coverage from 62.82% to 80%+" \
  --label "testing,priority:medium,size:medium,type:test" \
  --body "## 🎯 Objective
...
echo "============================================"
echo "✅ All 5 priority issues created successfully!"
echo "============================================"
